...
impl<T: Clone> IcmpPacket<T> {
    pub fn echo_reply(&self) -> IcmpPacket<T> {
        let (id, sequence_number) = match self.type_ {
            IcmpType::EchoRequest {id, sequence_number} => (id, sequence_number),
    ...
    pub fn echo_reply_packet(&self, src_mac: EthernetAddress, dst_mac: EthernetAddress, src_ip: Ipv4Address, dst_ip: Ipv4Address) -> EthernetPacket<Ipv4Packet<IcmpPacket<T>>> {
        let reply = self.echo_reply();
        EthernetPacket::new_ipv4(src_mac, dst_mac, Ipv4Packet::new_icmp(src_ip, dst_ip, reply))
    }
}
...
impl<T: AsRef<[u8]>> WriteOut for IcmpPacket<T> {
    ...
    fn write_out(&self, packet: &mut TxPacket) -> Result<(), ()> {
        ...
        match self.type_ {
            IcmpType::EchoRequest {id, sequence_number} | IcmpType::EchoReply {id, sequence_number} => {
                packet.push_u16(id)?;
